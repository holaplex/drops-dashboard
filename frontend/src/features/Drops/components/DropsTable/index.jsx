	...
	const Flag = ({ text, status }) => {
		return (
			<div className="flex items-center">
				<span className={`w-2 h-2  rounded-full mr-1 ${status ? 'bg-dropGreen' : 'bg-dropYellow'}`}></span>
				<span className={`text-xs ${status ? 'text-dropGreen' : 'text-dropYellow'}`}> {text}</span>
			</div>
		)
	}
							...
							<div key={drop.id} className="shadow-md rounded my-5 cursor-pointer hover:bg-slate-50" onClick={() => { handleReview(drop.id) }}>
								<div className="p-4 flex flex-row justify-between">
									...
									<div className="flex flex-row gap-5 items-center">
										<div className="text-dropGray text-xs">Golive : {drop?.go_live_date ? drop.go_live_date : 'N/A'}</div>
