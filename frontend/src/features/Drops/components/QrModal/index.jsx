...
const QRModal = ({url}) => {
	...
	return (
		<div className='flex w-full h-full justify-center items-center overflow-hidden absolute inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full'>
			<div className="w-400 p-8 rounded-16 bg-white z-50 overflow-auto" ref={ref}>
				<div className="flex flex-col gap-y-4">
					<div className="flex flex-row justify-between items-center">
						...
						<button
							...
							onClick={() => handleCopyClick("qr")}> {isQRCopied ? "Copied!" : "Copy QR code"} </button>
					</div>
					<div className="flex flex-row justify-between gap-x-2 items-center border p-2 rounded-lg ">
						<div>
							...
							<p className="text-xs font-normal whitespace-normal mr-1">{url}</p>
						</div>
						<button
							className="bg-dropCharcoal text-white h-10 px-6 py-2 rounded-lg"
							onClick={() => handleCopyClick("not-qr")}>{isCopied ? 'Copied!' : 'Copy'}</button></div>
				</div>
			</div>
		</div>
	)
