...
const Info = () => {
	...
	const { name, goLiveDate, accessible, discoverable, nfts, isSuccess, isError } = useSelector(dropSelector)
	...
	const handleUpdate = (value) => {
		if (drop_id) {
			setLoading(true)
			dispatch(update({ id: drop_id, accessible: value.accessible, discoverable: value.discoverable }))
		}
	}
			...
			<div className='flex flex-col md:flex-row p-1 md:p-36 md:gap-12'>
				<div className='md:w-2/3'>
					<div className='flex items-center mb-3'>
						<BiArrowBack size={24} className='text-dropGray mr-2 cursor-pointer' onClick={() => {
							navigate("/drops")
						}} />
						<h2 className='font-bold text-dropBlack text-3xl'>{name}</h2>
					</div>
					<span className='font-semibold text-dropBlack'> <span className='text-dropGray'> Go live: </span>{formatDateTime(goLiveDate)} </span>
								...
								<div key={index}>
									...
									<div className={`flex justify-between w-full my-4 `}>
										...
										<div className='flex items-center gap-4 font-bold'>
											...
											<button className='rounded-lg border-[1px] border-opacity-50 px-4 py-2 text-white bg-black' onClick={() => setIsComponentVisible(true)}>Share</button>
										</div>
									</div>
									<hr className="mt-6 text-dropGray bg-gray" />
								</div>
