class NftController < ApplicationController
  ...
  def upload_excel
    ...
    tabs.each do |tab|
      ...
      while i < last
        unless sheet.row(i)[0].nil?
          ...
          nft_image = GoogleService.get_drive_media(image_id, 'gallery', random_string)
          nft_final_media = GoogleService.get_drive_media(final_media_id, 'final', random_string)
          nft[:gallery_filename] = "/#{random_string}/#{nft_image[:name]}".gsub(' ', '_')
          nft[:final_filename] = "/#{random_string}/#{nft_final_media[:name]}".gsub(' ', '_')
    ...
    render json: { success: true, nfts: nfts },
           include: %i[school conference], status: :ok
  end

  def create_drop
    drop = NftDrop.create(params[:drop_name], params[:go_live_date])

    params[:nfts].each do |_nft|
      nft = Nft.create(_nft)
      nft[:drop_id] = drop.id
      nft.save
    ...
    render json: { success: true, message: 'Drop created' }, status: :ok
  end
